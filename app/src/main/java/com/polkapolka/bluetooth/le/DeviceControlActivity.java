...
*/
    ...
    private void handleData(String data) {
        ...
        if (data != null) {
            if (data.contains("AT+ADDR?")) {
                sendData("+ADDR:ANDROID89012 "); // this is our device identifier
            }
            if (data.equalsIgnoreCase("AT+RSSI?")) {
                sendData("AT+RSSI?"); //if it asks us for rssi, we ask back.
            }
            if (data.contains("Get")) {
                Log.d("mydebug", "Sending back rssi");
                sendData(data); // send back the rssi value
            }
            ...
            if (data.equalsIgnoreCase("AT")) {
                mBluetoothLeService.disconnect(); // disconnect
            }
